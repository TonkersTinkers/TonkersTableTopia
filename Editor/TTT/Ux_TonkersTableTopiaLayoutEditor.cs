...
public class Ux_TonkersTableTopiaLayoutEditor : Editor
{
    ...
    private void DrawTableSizeControls()
    {
        using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
        {
            ...
            usePercent = EditorGUILayout.ToggleLeft("Size By Percent", usePercent);
            EditorPrefs.SetBool("Ux_TableSize_UsePercent", usePercent);
            ...
            if (usePercent)
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (GUILayout.Button("Apply Percent", GUILayout.Width(140)))
                    {
                        Undo.RecordObject(rt, "Set Table Size %");
                        SetTableSizeByPercent(wPct / 100f, hPct / 100f);
            ...
            else
            {
                ...
                if (parent != null)
                {
                    EditorGUILayout.LabelField($"Parent: {parent.name}", EditorStyles.miniLabel);
                }
                w = EditorGUILayout.FloatField("Width (px)", Mathf.Max(0f, w));
                h = EditorGUILayout.FloatField("Height (px)", Mathf.Max(0f, h));
                using (new EditorGUILayout.HorizontalScope())
                {
                    GUILayout.FlexibleSpace();
                    if (GUILayout.Button("Apply Pixels", GUILayout.Width(140)))
                    {
                        Undo.RecordObject(rt, "Set Table Size px");
                        SetTableSizeManual(w, h);
                        table.ConvertAllSpecsToPercentages();
                        table.FlagLayoutAsNeedingSpaDay();
                        EditorUtility.SetDirty(table);
                        EditorUtility.SetDirty(rt);
    ...
    private void SetTableSizeByPercent(float wPct, float hPct)
    {
        var rt = table.GetComponent<RectTransform>();
        wPct = Mathf.Clamp01(wPct);
        hPct = Mathf.Clamp01(hPct);
        var aMin = rt.anchorMin;
        var aMax = rt.anchorMax;
        aMax.x = Mathf.Clamp01(aMin.x + wPct);
        aMax.y = Mathf.Clamp01(aMin.y + hPct);
        rt.anchorMin = aMin;
        rt.anchorMax = aMax;
    ...
    private void SetTableSizeManual(float width, float height)
    {
        var rt = table.GetComponent<RectTransform>();
    ...
    private void DrawWysiwyg()
    {
        ...
        float[] previewCol = BuildDistributed1D(
            cols,
            ...
            innerW,
            ensureAtLeastOneFlex: true
        );

        float[] previewRow = BuildDistributed1D(
            rows,
    ...
    private float[] BuildDistributed1D(int count, Func<int, float> getFixedOrPercent, float spacing, float inner, bool ensureAtLeastOneFlex)
    {
        ...
        {
            float v = (i < count) ? getFixedOrPercent(i) : 0f;
            if (v > 0f)
            {
                fixedPx[i] = v;
                fixedSum += v;
        ...
        if (flexCount > 0)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (flexShares[i] > 0) result[i] = fixedPx[i] + share;
                else if (result[i] <= 0f) result[i] = fixedPx[i];
            }
        }
        else
        {
            for (int i = 0; i < count; i++)
                if (result[i] <= 0f) result[i] = fixedPx[i];
            if (count > 0 && residual > 0f) result[count - 1] += residual;
        }

        if (ensureAtLeastOneFlex && count > 0)
        {
            bool allFixed = true;
            for (int i = 0; i < count; i++)
                if (flexShares[i] > 0 || perc[i] > 0f) { allFixed = false; break; }
            if (allFixed) result[count - 1] = Mathf.Max(1f, result[count - 1]);
        }
    ...
    private float[] ComputeColumnWidths(float totalW)
    {
        int n = table.totalColumnsCountHighFive;
        var result = new float[n];

        float spacingSum = Mathf.Max(0, n - 1) * table.sociallyDistancedColumnsPixels;
        float fixedSum = 0f;
        int flexCount = 0;

        for (int c = 0; c < n; c++)
        {
            float w = (c < table.fancyColumnWardrobes.Count && table.fancyColumnWardrobes[c].requestedWidthMaybePercentIfNegative > 0f)
                      ? table.fancyColumnWardrobes[c].requestedWidthMaybePercentIfNegative
                      : -1f;

            if (table.shareThePieEvenlyForColumns) w = -1f;
        ...
        float remain = Mathf.Max(0f, totalW - spacingSum - fixedSum);
        float share = flexCount > 0 ? remain / flexCount : 0f;

        for (int c = 0; c < n; c++)
            if (result[c] <= 0f) result[c] = share;

        return result;
    }

    private float[] ComputeRowHeights(float totalH)
    {
        int n = table.totalRowsCountLetTheShowBegin;
        var result = new float[n];

        float spacingSum = Mathf.Max(0, n - 1) * table.sociallyDistancedRowsPixels;
        float fixedSum = 0f;
        int flexCount = 0;

        for (int r = 0; r < n; r++)
        {
            float h = (r < table.snazzyRowWardrobes.Count && table.snazzyRowWardrobes[r].requestedHeightMaybePercentIfNegative > 0f)
                      ? table.snazzyRowWardrobes[r].requestedHeightMaybePercentIfNegative
                      : -1f;

            if (table.shareThePieEvenlyForRows) h = -1f;
        ...
        float remain = Mathf.Max(0f, totalH - spacingSum - fixedSum);
        float share = flexCount > 0 ? remain / flexCount : 0f;

        for (int r = 0; r < n; r++)
            if (result[r] <= 0f) result[r] = share;
    ...
    private void DrawHeaders(Rect top, Rect left, float[] colW, float[] rowH)
    {
        ...
        {
            var rr = new Rect(left.x, y, left.width, rowH[r]);
            EditorGUI.LabelField(rr, (r + 1).ToString(), EditorStyles.centeredGreyMiniLabel);
    ...
    private float[] ComputeCurrentColumnPercentages()
    {
        ...
        var pct = new float[n];
        float sum = 0f;
        for (int i = 0; i < n; i++) { pct[i] = Mathf.Clamp01(px[i] / avail); sum += pct[i]; }
    ...
    private void EnsureAtLeastOneFlexibleColumn()
    {
        bool hasFlex = false;
        for (int i = 0; i < table.totalColumnsCountHighFive && i < table.fancyColumnWardrobes.Count; i++)
            if (table.fancyColumnWardrobes[i].requestedWidthMaybePercentIfNegative <= 0f) { hasFlex = true; break; }

        if (!hasFlex && table.totalColumnsCountHighFive > 0)
            table.fancyColumnWardrobes[table.totalColumnsCountHighFive - 1].requestedWidthMaybePercentIfNegative = 0f;
    }

    private int FindSinkColumnIndex()
    {
        for (int i = table.totalColumnsCountHighFive - 1; i >= 0; i--)
            if (i < table.fancyColumnWardrobes.Count && table.fancyColumnWardrobes[i].requestedWidthMaybePercentIfNegative <= 0f)
                return i;
        return Mathf.Max(0, table.totalColumnsCountHighFive - 1);
    }
    ...
    private void EnsureAtLeastOneFlexibleRow()
    {
        bool hasFlex = false;
        for (int i = 0; i < table.totalRowsCountLetTheShowBegin && i < table.snazzyRowWardrobes.Count; i++)
            if (table.snazzyRowWardrobes[i].requestedHeightMaybePercentIfNegative <= 0f) { hasFlex = true; break; }

        if (!hasFlex && table.totalRowsCountLetTheShowBegin > 0)
            table.snazzyRowWardrobes[table.totalRowsCountLetTheShowBegin - 1].requestedHeightMaybePercentIfNegative = 0f;
    }

    private int FindSinkRowIndex()
    {
        for (int i = table.totalRowsCountLetTheShowBegin - 1; i >= 0; i--)
            if (i < table.snazzyRowWardrobes.Count && table.snazzyRowWardrobes[i].requestedHeightMaybePercentIfNegative <= 0f)
                return i;
        return Mathf.Max(0, table.totalRowsCountLetTheShowBegin - 1);
    }

    private void DrawTableTools()
    {
        ...
        using (new EditorGUILayout.HorizontalScope())
        {
            ...
            {
                EnsureColumnStylesSize();
                for (int i = 0; i < table.totalColumnsCountHighFive - 1; i++)
                    table.fancyColumnWardrobes[i].requestedWidthMaybePercentIfNegative =
                        Mathf.Max(1f, table.fancyColumnWardrobes[i].requestedWidthMaybePercentIfNegative);
                table.fancyColumnWardrobes[table.totalColumnsCountHighFive - 1].requestedWidthMaybePercentIfNegative = 0f;
                table.FlagLayoutAsNeedingSpaDay();
                EditorUtility.SetDirty(table);
            }
            ...
            {
                EnsureRowStylesSize();
                for (int i = 0; i < table.totalRowsCountLetTheShowBegin - 1; i++)
                    table.snazzyRowWardrobes[i].requestedHeightMaybePercentIfNegative =
                        Mathf.Max(1f, table.snazzyRowWardrobes[i].requestedHeightMaybePercentIfNegative);
                table.snazzyRowWardrobes[table.totalRowsCountLetTheShowBegin - 1].requestedHeightMaybePercentIfNegative = 0f;
                table.FlagLayoutAsNeedingSpaDay();
                EditorUtility.SetDirty(table);
            }
    ...
    {
        if (!HasSelection()) return;
        int r = Mathf.Min(selRow, selRow2);
    ...
    {
        if (!HasSelection()) return;
        int c = Mathf.Min(selCol, selCol2);
    ...
    private void UnmergeTopLeft()
    {
        ...
        table.UnmergeCellEveryoneGetsTheirOwnChair(r0, c0);
        table.FlagLayoutAsNeedingSpaDay();
    }
